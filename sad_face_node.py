...
class SadFaceNode:
    def __init__(self):
        self.sad_face_path = '/home/quori6/KityRainbowGoddessstuff/sad_face.gif'
        ...
        self.label.load(self.sad_face_path)
        print("GIF loaded. Frame count:", len(self.label.frames))

        rospy.init_node('sad_face_node', anonymous=True)
        self.finished = False

        threading.Thread(target=self.say_and_close).start()

    def say_and_close(self):
        text = "I feel like no one wants to hear from me."
        tts = gTTS(text=text, lang='en')
        audio_path = "/tmp/sad.mp3"
        tts.save(audio_path)
    ...
    def shutdown(self):
        if not self.finished:
            self.finished = True
            rospy.loginfo("Shutting down sad face node.")
            self.root.quit()
            self.root.destroy()
            rospy.signal_shutdown("Sad face node shutdown")
